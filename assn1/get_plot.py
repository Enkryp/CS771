...
def get_accuracy( X, y, w, b ):
    ...
    return np.average( get_renamed_labels( y ) == predictions )
################################
# Non Editable Region Starting #
################################
def get_features( X ):
...
################################
    ...
    X_test = get_features(Z_tst[:, :-1]) 
    Y_test = get_renamed_labels(Z_tst[:, -1]) 
...
################################
    while True:
        ...
        if t % spacing == 0:
            ...
            errorSeries.append(get_accuracy(X_test, Y_test, W, B))
            if totTime > timeout:
                plt.plot(timeSeries, errorSeries)
